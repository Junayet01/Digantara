...
# 1. Efficiently read TLE and create satellite objects using asynchronous I/O
...
# Load the TLE data asynchronously
satellites = asyncio.run(read_tle_async('30000sats.txt'))
...
def calculate_positions(satellites, start_date, minutes_interval=1, batch_size=10):
    ...
    for i in range(0, len(satellites), batch_size):
        ...
        for satellite in batch:
            e, r, v = satellite.sgp4_array(jd_times, fr_array)
            e = cp.asarray(e)
            r = cp.asarray(r)
            v = cp.asarray(v)
            valid_indices = cp.where(e == 0)[0]
